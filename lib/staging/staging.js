(function () {
  ...
  /** After layout export / Publish: hide top-left Live badge on the next live load. */
  function markOfficialLiveAfterExport() {
    try {
      window.localStorage.setItem(OFFICIAL_LIVE_STORAGE_KEY, "1");
    } catch (e) {
      /* ignore */
    }
  }
      ...
      .then(function (reachable) {
        ...
        return flushSiteStateToApi().then(function () {
          return exportLayoutBaked(api).then(function (bake) {
            if (!bake || !bake.ok) {
              return { ok: false, error: "export_failed", bake: bake };
            }
            markOfficialLiveAfterExport();
  ...
  function addStagingPublishControl() {
    ...
    btn.addEventListener("click", function (e) {
      ...
      publishSiteToLive().then(function (result) {
        ...
        if (result.error === "export_failed") {
          feedback.textContent =
            "Saved to SQLite, but layout export failed. Check dev API logs, then try Publish again.";
          return;
        }
