(function () {
  ...
  function addStagingPublishControl() {
    ...
    btn.setAttribute(
      "title",
      "Saves frame and gallery layout to SQLite, runs POST /api/export-layout (data/layout/*.json), copies gallery session prefs to published localStorage, then reloads without ?staging=1. Deploy static site + data/layout/ for official-live.",
    );
