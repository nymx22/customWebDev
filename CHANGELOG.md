...
- [upg] **Staging registry:** `lib/staging/staging.js` **Pages / galleries** dialog talks to the layout API (`GET /api/registry`, `POST`/`PATCH`/`DELETE` page & gallery) instead of `localStorage` keys `customdev_staging_pages_v1` / `customdev_staging_galleries_v1`. **Add page** / **Add gallery** use collapsible `<details>` (closed by default); page and gallery lists support **Edit** + **Save**/`PATCH` per row. Staging toolbar: **Staging**, **Publish galleries to live**, and **Pages / galleries** stay on one row (`lib/staging/staging.css`); publish feedback is single-line with ellipsis.
